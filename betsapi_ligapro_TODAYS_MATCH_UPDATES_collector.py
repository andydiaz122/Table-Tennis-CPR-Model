...
try:
    ...
    # ✅ --- FIX: Use current UTC time to determine which days to query ---
    # This ensures the script is synchronized with the API server's timezone.
    today_utc = datetime.now(pytz.utc) 
    
    for i in range(DAYS_TO_FETCH):
        # ✅ --- FIX: Base the query date on UTC time, not local time ---
        ...
        print(f"--- Processing Day: {current_date_to_query.strftime('%Y-%m-%d')} (UTC) {'(Re-checking for updates)' if is_today else ''} ---")
                ...
                try:
                    ...
                    if data and data.get('success') == 1 and data.get('results'):
                        ...
                        if match_time_unix:
                            ...
                            # Convert to the Eastern timezone for display in the CSV
