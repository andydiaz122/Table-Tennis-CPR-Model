...
META_MODEL_OUTPUT_FILE = "cpr_v7.4_meta_model.pkl"
...
try:
    ...
    # ADJUSTMENT: Rename columns to be safe, just like in the trainer.
    ...
    df_sequence_features = df[['Player 1 ID', 'Player 2 ID'] + features_to_sequence].copy()

    # ADJUSTMENT: Moved dropna() to be more specific and prevent dropping rows unnecessarily.
    ...
    for index, row in df_sequence_features.iterrows():
        ...
        # Create the leak-proof feature vector from P1's perspective
        p1_perspective_vector = np.array([
            row['P1_Win'], row['P1_Rest_Days'], 1.0 - row['P1_Win'],
            row['P2_Rest_Days'], row['H2H_P1_Win_Rate']
        ])
        if p1_id not in player_history: player_history[p1_id] = []
        player_history[p1_id].append(p1_perspective_vector)
        
        p2_perspective_vector = np.array([
            1.0 - row['P1_Win'], row['P2_Rest_Days'], row['P1_Win'],
            row['P1_Rest_Days'], 1.0 - row['H2H_P1_Win_Rate']
        ])
...
#     aligned_df['Rest_Advantage'] = aligned_df['P1_Rest_Days'] - aligned_df['P2_Rest_Days']
    aligned_df['Set_Comebacks_Advantage'] = aligned_df['P1_Rolling_Set_Comebacks_L20'] - aligned_df['P2_Rolling_Set_Comebacks_L20']
    
    X_gbm = aligned_df[['Vig_Free_Advantage', 'Win_Rate_Advantage', 'Pressure_Points_Advantage', 'Set_Comebacks_Advantage']]
...
#    X_meta = gbm_predictions.reshape(-1, 1)        # Temporarily use only the GBM predictions as features
#    X_meta = np.vstack([gbm_predictions, lstm_predictions]).T
    # - Give the meta-model the specialist predictions AND the market odds
    X_meta = np.c_[gbm_predictions, lstm_predictions, aligned_df['Vig_Free_Advantage'].values]

    print(f"\nTraining the meta-model on {len(X_meta)} samples...")
    meta_model = LogisticRegression(penalty='l2', C=1.0, solver='liblinear')
