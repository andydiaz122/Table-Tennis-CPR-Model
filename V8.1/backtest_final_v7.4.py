...
ANALYSIS_LOG_FILE = "backtest_log_for_analysis_v7.4_filtered.csv"
EQUITY_CURVE_FILE = "equity_curve_v7.4_filtered.png"
...
EDGE_THRESHOLD_MIN = 0.30
EDGE_THRESHOLD_MAX = 0.45
...
try:
    ...
    # 🌟 FIX: Use the 'na_values' parameter to treat '-' as a missing value (NaN)
    # 🌟 and explicitly set low_memory=False to help pandas handle mixed types better.
    df = pd.read_csv(
        FINAL_DATASET_FILE,
        na_values=['-'], 
    ...
    # ADJUSTMENT: Added rename call to prevent KeyError.
    df.rename(columns={'P1 Pressure Points': 'P1_Pressure_Points', 'P2 Pressure Points': 'P2_Pressure_Points'}, inplace=True, errors='ignore')
    df.dropna(subset=['P1_Win', 'Kickoff_P1_Odds', 'Kickoff_P2_Odds', 'P1_Pressure_Points', 'P2_Pressure_Points'], inplace=True)
    ...
    print(f"\n--- Generating New Symmetrical Log with Performance Filters ---")
    ...
    for index, match in tqdm(backtest_df.iterrows(), total=backtest_df.shape[0]):
        ...
        p1_games_total = history_df[(history_df['Player 1 ID'] == p1_id) | (history_df['Player 2 ID'] == p1_id)]
        p2_games_total = history_df[(history_df['Player 1 ID'] == p2_id) | (history_df['Player 2 ID'] == p2_id)]
        p1_history_count = len(p1_games_total)
        p2_history_count = len(p2_games_total)

        h2h_df_check = history_df[((history_df['Player 1 ID']==p1_id)&(history_df['Player 2 ID']==p2_id))|((history_df['Player 1 ID']==p2_id)&(history_df['Player 2 ID']==p1_id))]
        ...
        # --- Model Prediction ---
        # Note: Rest_Advantage is not used by the current model version, so it is omitted here.
        gbm_features = pd.DataFrame([{'Win_Rate_Advantage': win_rate_advantage, 'Pressure_Points_Advantage': pressure_points_advantage, 'Set_Comebacks_Advantage': set_comebacks_advantage}])
    ...
    #    h2h_p1_win_rate = p1_h2h_wins / len(h2h_df) if len(h2h_df) > 0 else 0.5
        ...
        if edge_p1 > EDGE_THRESHOLD_MIN and edge_p1 < EDGE_THRESHOLD_MAX:
            # APPLY FILTERS: Only bet if all conditions are met
            if (win_rate_advantage > 0 and 
                h2h_p1_win_rate > 0.5 and
                (1.5 <= p1_market_odds < 2.0) and
                (p1_market_odds - 1) > MIN_ODDS_DENOMINATOR):
        ...
        #        if p1_market_odds >= CONTRARIAN_ODDS_CEILING:
            ...
            if (win_rate_advantage < 0 and 
                h2h_p1_win_rate < 0.5 and 
                (1.5 <= p2_market_odds < 2.0) and
                (p2_market_odds - 1) > MIN_ODDS_DENOMINATOR):
    ...
    print("\n--- Final Back-test Summary (PERFORMANCE FILTERS APPLIED) ---")
    ...
    if len(bankroll_history_per_bet) > 1:
        ...
        history_df['Bankroll'] = pd.to_numeric(history_df['Bankroll'], errors='coerce')
        ...
        plt.plot(range(len(history_df)), history_df['Bankroll'])
    ...
    if bet_log:
        ...
        print(f"\n✅ Filtered analysis log saved to '{ANALYSIS_LOG_FILE}'")
