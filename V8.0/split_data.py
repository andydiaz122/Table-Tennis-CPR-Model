...
# --- EMBARGO CONFIGURATION (from feature-research d474e96) ---
# Set to 0 to disable (default), or 24 for 24-hour embargo between train/test
# Embargo prevents potential data leakage from same-day matches
EMBARGO_HOURS = 0


# --- 2. Embargo Split Function ---
...
#    df = pd.read_csv(INPUT_FILE, parse_dates=['Date'], date_format='mixed', low_memory=False)
    ...
    # --- Use embargo-aware split if enabled ---
    if EMBARGO_HOURS > 0:
        ...
        print(f"  Embargo requirement met: {'Yes' if embargo_stats['embargo_met'] else 'No'}")
        print(f"  Overlapping players at boundary: {embargo_stats['overlapping_players_count']}")
    else:
        # Original behavior: simple split without embargo
