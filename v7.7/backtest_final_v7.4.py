...
# Finalized output file names for this version
ANALYSIS_LOG_FILE = "backtest_log_for_analysis_final_v7.7.csv"
EQUITY_CURVE_FILE = "equity_curve_final_v7.7.png"

INITIAL_BANKROLL = 1000
KELLY_FRACTION = 0.06 # Using your tuned value
TRAIN_SPLIT_PERCENTAGE = 0.80
ROLLING_WINDOW = 10
SEQUENCE_LENGTH = 5

# Finalized Strategic Filters
...
try:
    print("--- Loading Data and Models for FINAL v7.7 Back-test ---")
    ...
    print(f"Loaded {len(df)} total matches. Training history on {len(train_df)}, back-testing on {len(backtest_df)} matches.")
    ...
    print(f"\n--- Running FINAL Leak-Proof Back-test (v7.7) ---")
    
    # Pre-populate player history using the training data to ensure point-in-time correctness
    player_history = {}
    print("Pre-populating player history from training data...")
    for index, row in tqdm(train_df.iterrows(), total=train_df.shape[0]):
        p1_id, p2_id = row['Player 1 ID'], row['Player 2 ID']
        
        # We only need to store the raw vectors for the LSTM
    ...
    for index, match in tqdm(backtest_df.iterrows(), total=backtest_df.shape[0]):
        ...
        # --- ON-THE-FLY FEATURE ENGINEERING (for GBM & Filters) ---
        ...
        # --- Create LSTM sequences from the point-in-time player_history dictionary ---
        p1_hist = player_history.get(p1_id, [])
        p2_hist = player_history.get(p2_id, [])
        
        if len(p1_hist) < SEQUENCE_LENGTH or len(p2_hist) < SEQUENCE_LENGTH:
            continue
            
        p1_seq = p1_hist[-SEQUENCE_LENGTH:]
        p2_seq = p2_hist[-SEQUENCE_LENGTH:]
        ...
        # --- MERGED & FINALIZED BETTING LOGIC (from v7.4) ---
        ...
        if bet_details:
            ...
            # On-the-fly H2H for logging purposes
        ...
        # Update player history AFTER processing the current match to prevent data leakage
        p1_perspective_vector = [match['P1_Win'], match['P1 Pressure Points'], match['P1_Rest_Days'], 1.0 - match['P1_Win'], match['P2 Pressure Points'], match['P2_Rest_Days'], match['H2H_P1_Win_Rate']]
        if p1_id not in player_history: player_history[p1_id] = []
        player_history[p1_id].append(p1_perspective_vector)

        p2_perspective_vector = [1.0 - match['P1_Win'], match['P2 Pressure Points'], match['P2_Rest_Days'], match['P1_Win'], match['P1 Pressure Points'], match['P1_Rest_Days'], 1.0 - match['H2H_P1_Win_Rate']]
        if p2_id not in player_history: player_history[p2_id] = []
        player_history[p2_id].append(p2_perspective_vector)
