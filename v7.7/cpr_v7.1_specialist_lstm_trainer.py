...
try:
    ...
    df.dropna(inplace=True) # Drop rows with any missing base features

    # --- 3. Sequence Creation with RAW Point-in-Time Features ---
    print("Creating match sequences with raw, point-in-time features to prevent data leakage...")
    player_history = {}
    X_p1_sequences, X_p2_sequences, y_train = [], [], []

    # Define the RAW features that represent a player's performance in a SINGLE match
    # The LSTM will learn momentum from a sequence of these vectors.
    # NOTE: We've replaced rolling averages with raw win/loss and raw pressure points.
    features_to_use = [
        'P1_Win', 'P1 Pressure Points', 'P1_Rest_Days',
        'P2 Pressure Points', 'P2_Rest_Days',
        'H2H_P1_Win_Rate'
    ]
    # Also need player IDs for tracking history
    df_sequence_features = df[['Player 1 ID', 'Player 2 ID'] + features_to_use]
    ...
    for index, row in df_sequence_features.iterrows():
        ...
        if len(p1_hist) >= SEQUENCE_LENGTH and len(p2_hist) >= SEQUENCE_LENGTH:
            # Append the LAST N matches from their history. This is their momentum coming into the current match.
            X_p1_sequences.append(p1_hist[-SEQUENCE_LENGTH:])
            X_p2_sequences.append(p2_hist[-SEQUENCE_LENGTH:])
            # The target is the outcome of the CURRENT match
            y_train.append(row['P1_Win'])

        # Create the raw feature vector for P1's perspective FOR THIS MATCH
        # This vector will be added to history to be used for FUTURE predictions
        p1_perspective_vector = [
            row['P1_Win'],              # My result in this match (1.0 or 0.0)
            row['P1 Pressure Points'],  # My raw pressure points
            row['P1_Rest_Days'],        # My rest days
            1.0 - row['P1_Win'],        # Opponent's result (P2_Win)
            row['P2 Pressure Points'],  # Opponent's raw pressure points
        ...
        # Create the raw feature vector for P2's perspective FOR THIS MATCH
        p2_perspective_vector = [
            1.0 - row['P1_Win'],        # My result in this match (P2_Win)
            row['P2 Pressure Points'],  # My raw pressure points
            row['P2_Rest_Days'],        # My rest days
            row['P1_Win'],              # Opponent's result
            row['P1 Pressure Points'],  # Opponent's raw pressure points
    ...
    # Apply LSTM layers to inputs
    lstm_p1 = lstm_layer1(input_p1)
    lstm_p2 = lstm_layer2(input_p2)

    # Add Dropout after LSTM layers
