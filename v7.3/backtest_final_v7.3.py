...
# --- NEW: Output file for the final log ---
FINAL_LOG_FILE = "backtest_log_final_v7.3.csv" 
...
try:
    print("--- Loading Data and Models for FINAL v7.3 Back-test ---")
    ...
    print(f"\n--- Running FINAL Back-test with v7.3 Intelligent Filters ---")
    ...
    for index, match in tqdm(backtest_df.iterrows(), total=backtest_df.shape[0]):
        ...
        if edge_p1 > EDGE_THRESHOLD_MIN and edge_p1 < EDGE_THRESHOLD_MAX:
            ...
            if h2h_p2_win_rate >= H2H_DISADVANTAGE_THRESHOLD:
                bet_count += 1
    ...
    print("\n--- Final Back-test Summary (v7.3 Intelligent Filters) ---")
    ...
    if bet_log:
        ...
        log_df.to_csv(FINAL_LOG_FILE, index=False)
        print(f"\n✅ Final, filtered bet log saved to '{FINAL_LOG_FILE}'")
