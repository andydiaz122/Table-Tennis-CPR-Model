...
# --- 1. Configuration ---
TEST_FEATURES_FILE = "testing_dataset.csv"
ODDS_FILE = "backtest_data_with_odds.csv"

# --- NEW: Using the re-trained v6.2 models ---
...
KELLY_FRACTION = 0.25
EDGE_THRESHOLD_MIN = 0.001 # Bet if edge is > .1%
EDGE_THRESHOLD_MAX = 0.32 # Bet if edge is < 32%

# --- 2. Load All Models and Data ---
try:
    print("--- Loading All Models and Data for Final Back-test ---")
    df_features = pd.read_csv(TEST_FEATURES_FILE)
    df_odds = pd.read_csv(ODDS_FILE)

    # Merge the features and odds data on the Match ID
    backtest_df = pd.merge(df_features, df_odds[['Match ID', 'P1_Simulated_Odds']], on='Match ID', how='inner')
    backtest_df.dropna(inplace=True)
    backtest_df['Date'] = pd.to_datetime(backtest_df['Date'])
    backtest_df.sort_values(by='Date', inplace=True)
    print(f"Loaded and merged data for {len(backtest_df)} test matches.")
    ...
    print("All v6.2 models loaded successfully.")

    # --- 3. Feature Engineering for the Test Set ---
    # Create the 'Advantage' features required by the GBM model
    ...
    # B. LSTM Predictions (Recreate sequences from the test set)
    player_history = {}
    ...
    # Filter all data to only the matches where sequences could be built
    ...
    print("\n--- Running Final Back-test Simulation (v6.4 Models) ---")
