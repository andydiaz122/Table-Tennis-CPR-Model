...
def analyze_timeline(timeline_data, scores_data):
    ...
    for set_num, set_scores in scores_data.items():
        stats['p1_total_points'] += int(set_scores.get('home', 0))
        stats['p2_total_points'] += int(set_scores.get('away', 0))
    ...
    for set_num, points in sets.items():
        ...
        if set_info:
            set_winner_p1 = int(set_info.get('home', 0)) > int(set_info.get('away', 0))
