...
try:
    ...
    for index, match in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        if not p1_rolling_games.empty:
            p1_wins = p1_rolling_games.apply(lambda r: 1 if (r['Player 1 ID'] == p1_id and r['P1_Win'] == 1) or (r['Player 2 ID'] == p1_id and r['P1_Win'] == 0) else 0, axis=1).sum()
            p1_win_rate = p1_wins / len(p1_rolling_games)
        ...
        if not p2_rolling_games.empty:
            p2_wins = p2_rolling_games.apply(lambda r: 1 if (r['Player 1 ID'] == p2_id and r['P1_Win'] == 1) or (r['Player 2 ID'] == p2_id and r['P1_Win'] == 0) else 0, axis=1).sum()
            p2_win_rate = p2_wins / len(p2_rolling_games)
    ...
    # Note: This script does not add betting odds. Your workflow should handle merging those in later.
