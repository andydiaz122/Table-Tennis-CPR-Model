...
#    df['Date'] = pd.to_datetime(df['Date'])
    ...
    # --- DEFINITIVE FIX: Initialize variables for daily bankroll logic ---
    daily_betting_bankroll = INITIAL_BANKROLL
    last_processed_date = None

    for index, match in tqdm(backtest_df.iterrows(), total=backtest_df.shape[0]):
        
        # --- DEFINITIVE FIX: Check for a new day and lock in the bankroll ---
        current_date = match['Date'].date()
        if last_processed_date != current_date:
            daily_betting_bankroll = bankroll # Lock in the bankroll at the start of the day
        ...
        # (Feature engineering and prediction logic remains the same...)
        ...
        gbm_features = pd.DataFrame([{'Win_Rate_Advantage': p1_win_rate - p2_win_rate, 'Pressure_Points_Advantage': p1_pressure_points - p2_pressure_points, 'Player 1 ID': p1_id, 'Player 2 ID': p2_id}])
        ...
        # Bankrupt check
        if bankroll <= 0:
            continue # Can't bet if bankroll is zero or less, so skip to next match
        ...
        if edge_p1 > EDGE_THRESHOLD_MIN and edge_p1 < EDGE_THRESHOLD_MAX:
            if (p1_market_odds - 1) > MIN_ODDS_DENOMINATOR:
                ...
                # --- DEFINITIVE FIX: Use the locked-in daily bankroll for staking ---
                stake = daily_betting_bankroll * capped_fraction
                
                total_staked += stake
                profit = stake * (p1_market_odds - 1) if actual_winner == 1 else -stake
                bankroll += profit # Main bankroll is updated for tracking, but not used for today's stakes
        ...
        if bet_details:
            ...
            log_entry = {'Match_ID': match['Match ID'],'Date': match['Date'].strftime('%Y-%m-%d'),'Player_1': match['Player 1'],'Player_2': match['Player 2'],'P1_History': p1_history_count,'P2_History': p2_history_count,'H2H_P1_Win_Rate': h2h_p1_win_rate,'Win_Rate_Advantage': p1_win_rate - p2_win_rate}
