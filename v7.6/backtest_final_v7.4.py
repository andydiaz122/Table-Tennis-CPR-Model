...
# --- CORRECTED & REFINED STRATEGIC FILTERS (Based on Forensic Report Recommendations) ---
# Targets 'High' (25%+) and 'Mega' (50%+) edge categories, which were most profitable.
EDGE_THRESHOLD_MIN = 0.25
# Avoids betting on longshots, which were unprofitable (-8.82% ROI).
ODDS_THRESHOLD_MAX = 3.0
# Ensures we only bet on players with an even (0.5) or winning (>0.5) H2H record. Avoids the catastrophic -53.5% ROI category.
H2H_MIN_WIN_RATE = 0.5
# Sets a minimum win rate advantage to qualify as 'Better Form'. Avoids 'Even Form' (-4.35% ROI) and 'Worse Form' (-4.75% ROI).
MIN_FORM_ADVANTAGE = 0.0
# Focuses on 'Veteran' players, where the model performance is most reliable (+13.24% ROI on ~14.5k bets).
MIN_VETERAN_HISTORY = 50


# --- 2. Load Models and Prepare Data ---
try:
    print("--- Loading Data and Models for FINAL v7.4 Back-test (Corrected Filters) ---")
    ...
    print(f"\n--- Running FINAL Back-test with Corrected Forensic Filters ---")
    ...
    for index, match in tqdm(backtest_df.iterrows(), total=backtest_df.shape[0]):
        ...
        p1_conditions = [
            ...
            h2h_p1_win_rate >= H2H_MIN_WIN_RATE,
            win_rate_advantage > MIN_FORM_ADVANTAGE, # CRITICAL FIX: Only bet on player with strictly BETTER form.
            p1_history_count >= MIN_VETERAN_HISTORY, # NEW: Ensure player is a veteran.
            p2_history_count >= MIN_VETERAN_HISTORY  # NEW: Ensure opponent is also a veteran for data quality.
        ]
        ...
        p2_conditions = [
            ...
            (1 - h2h_p1_win_rate) >= H2H_MIN_WIN_RATE,
            (-win_rate_advantage) > MIN_FORM_ADVANTAGE, # CRITICAL FIX: Only bet on player with strictly BETTER form.
            p1_history_count >= MIN_VETERAN_HISTORY, # NEW: Ensure player is a veteran.
            p2_history_count >= MIN_VETERAN_HISTORY  # NEW: Ensure opponent is also a veteran for data quality.
        ]
    ...
    print("\n--- Final Back-test Summary (Corrected Forensic Filters) ---")
